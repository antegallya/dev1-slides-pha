...
\def\acayear{2021--2022}
\date[\acayear]{Année académique \acayear}
...
\begin{frame}[fragile]{Concaténation}
  ...
  En java, l'opérateur de concaténation est \pc{+} quand au moins l'un des deux
  opérandes est un \pc{String}.
...
\begin{frame}[fragile]{Parcours de tableau}
\framesubtitle{\emph{classique}}
\begin{java}
int[] someArray = /* ... */;
for (int index = 0; index < someArray.length; ++index) {
  // do smth with someArray[index]
}
\end{java}
\end{frame}

\begin{frame}[fragile]{Parcours de tableau}
\framesubtitle{\emph{foreach} -- \emph{enhanced for}}
\begin{java}
int[] someArray = /* ... */;
...
\begin{frame}[fragile]{Parcours partiel}
\framesubtitle{Est-ce que le tableau contient un zero ?}
\pause
\begin{java}
...
\begin{frame}{Taille physique vs taille logique de tableau}
  \begin{block}{Problème}
    \structure{Problème} : lire des entrées sur \emph{stdin} (entrée standard) et les stocker dans un tableau (pour traitement ultérieur).
  \end{block}
  
  \pause

  Voir \texttt{exemples/InputArray.java}

  \begin{definition}[Taille physique/logique d'un tableau]
    La taille \structure{physique} d'un tableau est le nombre de cases qu'il
    contient (qui ont été \emph{allouées}). La taille \structure{logique} est le
    nombre de cases actuellement \emph{utilisées}.
  \end{definition}
\end{frame}

\end{hideedit}

\section{Algorithmes d'insertion et de recherche}
\begin{frame}{Situation d'exemple}
  Une école organise un événement libre et gratuit pour les étudiant·e·s, \structure{sous réserve d'inscription}.
...
\begin{frame}{\pc{Registration}}
  Nous allons stocker les matricules dans un tableau. Introduisons deux variables :
  \begin{itemize}
    \item \pc{registered} : le tableau des matricules
    \item \pc{nRegistered} : le nombre d’étudiant·es actuellement inscrit·es (la taille logique du tableau)
  \end{itemize}
...
\subsection{Insertion dans un tableau non trié}
\begin{frame}{Tableau non trié}
  Les valeurs sont stockées dans le tableau  dans un ordre quelconque.
...
\begin{frame}{Insertion dans un tableau non trié}
  \framesubtitle{\pc{Subscribe}}
  Pour inscrire un étudiant ou une étudiante, il suffit de l’ajouter
  à la suite dans le tableau.
...
\begin{frame}{Subscribe --- algorithme}
  \begin{langagenaturel}
    algorithm register(registered, nRegistered, number)\\
    \tab registered[nRegistered] = number\\
    \tab incrémenter nRegistered
  \end{langagenaturel}
\end{frame}

\begin{frame}[fragile]{Subscribe --- java}
  En Java, passer \pc{nRegistered} en paramètre d'entrée-sortie est
  impossible \(\rightarrow\) en valeur de retour.
...
\begin{frame}[fragile]{Chercher une valeur dans un tableau non trié}
  \framesubtitle{\pc{check} --- while}
  \begin{java}
...
\begin{frame}[fragile]{Chercher une valeur dans un tableau non trié}
  \framesubtitle{\pc{check} --- for}
  \begin{java}
public static int check(
        int[] registered, int nRegistered, int number){
    for(int i = 0; i < nRegistered, ++i) {
        if (registered[i] == number) {
            return i
        }
    }
    return -1;
...
\begin{frame}{Supprimer une valeur dans un tableau non trié}
  \framesubtitle{\pc{Unsubscribe}}

\end{frame}

\begin{frame}{Tableaux triés}
  \framesubtitle{Insertion, recherche, suppression}
  
  Pour la dichotomie : introduction avec \pc{NumberHuntGame}.

  Discussions et production de la classe \pc{RegistrationSorted}.

  Voir les fichiers \pc{NumberHuntGame.java} et \pc{RegistrationSorted.java} du dépôt \url{https://git.esi-bru.be/phauweele/dev1-pha-netbeans}
\end{frame}

\begin{frame}{Introduction à la complexité}
  Discutée pendant la production du code de la dichotomie.

  On a vu que la dichotomie est en \(O(\log_2(n))\) avec \(n\) la taille de \emph{l'espace de recherche}.
\end{frame}
